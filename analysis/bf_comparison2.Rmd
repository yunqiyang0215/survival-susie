...
for (i in 1:length(censor_lvl)){
  for (j in 1:length(af)){
    ...
    legend("bottomleft", legend = c("BF.lp", 'BF.wf'), col = c(1, "#00CCFF"), pch = 20, cex = 0.8)
  }
}
...
for (i in 1:length(censor_lvl)){
  for (j in 1:length(af)){
    ...
    dat = t(sapply(indx, function(x) res$compute_bf.result[[x]]))
    plot(x = dat[, 'lbf.numerical'], y = dat[, 'lbf.laplace'], pch = 20, main = paste0("censor=",censor_lvl[i], ", MAF=", af[j]),
         ylim = c(min(dat[,3:5])-1, max(dat[,3:5])+1), cex = 0.8,
         xlab = "BF.gh", ylab = "Other BFs")
    points(dat[, 'lbf.numerical'], dat[,'lbf.wakefeld'], col = "#00CCFF", pch = 20, cex = 0.8)
    abline(a = 0, b = 1, col = "grey")
    legend("topleft", legend = c("BF.lp", 'BF.wf'), col = c(1, "#00CCFF"), pch = 20, cex = 0.8)
  }
}
```
