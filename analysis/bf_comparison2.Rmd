...
for (i in 1:length(censor_lvl)){
  for (j in 1:length(af)){
    ...
    diff1 = dat2$lbf.numerical - dat2$lbf.wakefeld
    diff2 = dat2$lbf.numerical - dat2$lbf.laplace
    ...
    points(dat2$n[indx], diff2[indx], col = "green", cex = 0.8, pch = 20)
    legend("bottomleft", legend = c("BF.lp", 'BF.wf'), col = c(1, "green"), pch = 20, cex = 0.8)
  }
}
```
...
for (i in 1:length(censor_lvl)){
  for (j in 1:length(af)){
    indx = which(dat2$censor_lvl == censor_lvl[i] & dat2$allele_freq == af[j] & dat2$n == 1e4)
    diff1 = dat2$lbf.numerical - dat2$lbf.wakefeld
    diff2 = dat2$lbf.numerical - dat2$lbf.laplace
    ...
    points(dat2$bhat[indx], diff2[indx], col = "green", cex = 0.8, pch = 20)
    legend("bottomleft", legend = c("BF.lp", 'BF.wf'), col = c(1, "green"), pch = 20, cex = 0.8)
  }
}
...
for (i in 1:length(censor_lvl)){
  for (j in 1:length(af)){
    indx = which(dat2$censor_lvl == censor_lvl[i] & dat2$allele_freq == af[j] & dat2$n == 1e5)
    diff1 = dat2$lbf.numerical - dat2$lbf.wakefeld
    diff2 = dat2$lbf.numerical - dat2$lbf.laplace
    ymin = min(diff1[indx], diff2[indx]) - 0.5
    ymax = max(diff1[indx], diff2[indx]) + 0.5
