...
pdf("/project2/mstephens/yunqiyang/surv-susie/survival-susie/output/bf_defense.pdf", width = 8, height = 18)
par(mfrow = c(6,3))
...
for (i in 1:length(censor_lvl)){
  for (j in 1:length(af)){
    ...
    plot(x = dat[, 'lbf.numerical'], y = dat[, 'lbf.laplace'], pch = 20, main = paste0("censor=",censor_lvl[i], ", MAF=", af[j]),
         ylim = c(min(dat[,c(4:6)])-1, max(dat[, c(4:6)])+1), cex = 0.8, col = "#26B300",
