...
An example data where susie doesn't perform well. The true number of effect here is 1. When L=1, the result looks good. When setting L=5, the result looks weird. 
...
```{r}
fit_coxph <- ser_from_univariate(surv_uni_fun)
fit <- ibss_from_ser(X, y, L = 3, prior_variance = 1., prior_weights = rep(1/p, p), tol = 1e-3, maxit = maxiter, estimate_intercept = TRUE, ser_function = fit_coxph)
...
```{r}
fit_coxph <- ser_from_univariate(surv_uni_fun)
fit <- ibss_from_ser(X, y, L = 10, prior_variance = 1., prior_weights = rep(1/p, p), tol = 1e-3, maxit = maxiter, estimate_intercept = TRUE, ser_function = fit_coxph)
