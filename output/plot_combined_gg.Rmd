...
pip.susie.gtex = unlist(lapply(indx, function(x) susie$susie.pip[[x]]))
pip.svb.gtex = unlist(lapply(indx, function(x) svb$svb.pip[[x]]))
pip.bvsnlp.gtex = unlist(lapply(indx, function(x) bvsnlp$bvsnlp.pip[[x]]))
pip.r2b.gtex = unlist(lapply(indx, function(x) r2b$r2b.pip[[x]]))
pip.rss.gtex = unlist(lapply(indx, function(x) rss$rss.pip[[x]]))
is_effect.gtex = unlist(lapply(indx, function(x) susie$simulate.is_effect[[x]]))
...
indx = which(susie$simulate.cor_type == "real")
pip.susie.ukb = unlist(lapply(indx, function(x) susie$susie.pip[[x]]))
pip.svb.ukb = unlist(lapply(indx, function(x) survsvb$svb.pip[[x]]))
pip.bvsnlp.ukb = unlist(lapply(indx, function(x) bvsnlp$bvsnlp.pip[[x]]))
pip.r2b.ukb = unlist(lapply(indx, function(x) r2b$r2b.pip[[x]]))
pip.rss.ukb = unlist(lapply(indx, function(x) rss$susie_rss.pip[[x]]))
is_effect.ukb = unlist(lapply(indx, function(x) susie$simulate.is_effect[[x]]))
...
# Prepare all data into one combined data frame
pip_data <- rbind(
  prepare_pip_data(pip.susie.gtex, pip.svb.gtex, is_effect.gtex, "survival.svb", "GTEx simulation"),
  prepare_pip_data(pip.susie.gtex, pip.bvsnlp.gtex, is_effect.gtex, "BVSNLP", "GTEx simulation"),
  prepare_pip_data(pip.susie.gtex, pip.r2b.gtex, is_effect.gtex, "R2BGLiMS", "GTEx simulation"),
  prepare_pip_data(pip.susie.gtex, pip.rss.gtex, is_effect.gtex, "SuSiE.RSS", "GTEx simulation"),
  prepare_pip_data(pip.susie.ukb, pip.svb.ukb, is_effect.ukb, "survival.svb", "UKB simulation"),
  prepare_pip_data(pip.susie.ukb, pip.bvsnlp.ukb, is_effect.ukb, "BVSNLP", "UKB simulation"),
  prepare_pip_data(pip.susie.ukb, pip.r2b.ukb, is_effect.ukb, "R2BGLiMS", "UKB simulation"),
  prepare_pip_data(pip.susie.ukb, pip.rss.ukb, is_effect.ukb, "SuSiE.RSS", "UKB simulation")
)
...
ggplot(pip_data) +
  ...
  geom_point(data = subset(pip_data, is_effect == 0), 
             aes(x = pip1, y = pip2), color = "darkgrey", size = 1) +
  # Red dots on top
  geom_point(data = subset(pip_data, is_effect == 1), 
             aes(x = pip1, y = pip2), color = "red", size = 0.75) +
...
ggsave("/project2/mstephens/yunqiyang/surv-susie/survival-susie/output/pip_combined_ggplot.pdf", width = 16, height = 6, dpi = 300)
