...
compute_multiple_lbf <- function(x, y, o, prior_variance, ...){
  ...
  return(list(zscore = zscore, bhat = bhat, sd = sd,
              lbf=lbf, lbf.corr = lbf.corr, time.lbf.corr = (t2 - t1)[3]))
}
...
# compute other lbf
res.lbf = compute_multiple_lbf(x = dat$X[,1], y = survT, o = rep(0, n), prior_variance = prior_sd^2)
result = c(res.lbf$zscore, res.lbf$bhat, res.lbf$sd, res.lbf$lbf, res.lbf$lbf.corr, lbf.numerical)
names(result) = c("zscore", "bhat", "sd", "lbf.wakefeld", "lbf.laplace", "lbf.numerical")
